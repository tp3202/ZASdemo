...
$("#startTracking_start").live('click', function(){
  ...
  /*  watch_id = navigator.geolocation.watchPosition(
    ...
    $("#startTracking_status").html("position test: <strong>" + track_id + "</strong>");
});
...
$("#startTracking_stop").live('click', function(){
	...
	window.localStorage.setItem(track_id, tracking_data);
...
$('#history').live('pageshow', function () {
	...
	$("#tracks_recorded").html("<strong>" + tracks_recorded + "</strong> recorded");
...
//tracking_data
	...
	var myLat = tracking_data[0].coords.latitude;
	var myLong=	tracking_data[0].coords.longitude;

  var xmlHttp = null;

    xmlHttp = new XMLHttpRequest();
    xmlHttp.open( "GET", "http://api.geonames.org/findNearbyPostalCodes?lat=" + myLat + "&lng=" + myLong + "&username=terrenceis", false );
    xmlHttp.send( null );
